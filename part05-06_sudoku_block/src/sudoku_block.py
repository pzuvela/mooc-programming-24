...
def block_correct(sudoku: list, row_no: int, column_no: int) -> bool:
    filtered_flattened_sudoku_board = []
    for row in sudoku[row_no:row_no+3]:
        filtered_flattened_sudoku_board += [el for el in row[column_no:column_no+3] if el != 0]
    return len(filtered_flattened_sudoku_board) == len(set(filtered_flattened_sudoku_board))
...
if __name__ == "__main__":
    ...
    to the row and column indexes of a single square, as its arguments. Rows and columns 
    are indexed from 0.
    The function should return True or False depending on whether the 3 by 3 block to the 
    right and down from the given indexes is filled in correctly. That is, whether the 
    block contains each of the numbers 1 to 9 at most once.
    Notice that this function does not strictly follow the rules of sudoku. In a real game of 
    sudoku there are only 9 blocks to check, and these are located at indexes 
